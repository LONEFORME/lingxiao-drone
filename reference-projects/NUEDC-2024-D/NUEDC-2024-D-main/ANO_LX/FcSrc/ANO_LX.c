...
static inline void RC_Data_Task(float dT_s)
{
	...
	if ( rc_in.fail_safe == 0)
	{
		...
		else if (rc_in.rc_ch.st_data.ch_[ch_5_aux1] < 1700)
		{
			LX_Change_Mode(2);
			mod_f[0] = 2;
//			t265.ctrl_mode=0;		//不管如何都不进入纯程序控制模式 BY LHB
			LX_FC.sped_state=0;//光流定点
		}
		else
		{
//			LX_Change_Mode(3);
//			mod_f[0] = 3;
			LX_Change_Mode(2);
			mod_f[0] = 2;
//			t265.ctrl_mode=1;			//不管如何都不进入纯程序控制模式 BY LHB
			LX_FC.sped_state=1;//速度传感器来源切换成T265，且实时控制使能
			if(rc_in.rc_ch.st_data.ch_[ch_8_aux4]>1400)
			LX_FC.sped_state=0;//速度传感器来源切换成光流，且实时控制失能
		}
		//有切换模式时执行
		if (mod_f[1] != mod_f[0])
		{
		...
		if (mod_f[2] != 0)
		{
			...
			else
			{
				...
				{
					//执行返航
//					tmp = OneKey_Return_Home();//取消返航罗汉斌
					tmp=1;
				}
				else
				{
...
//		}
		...
		//实时XYZ-YAW期望速度(实时控制帧)
		//根据当前控制模式进行赋值LHB
		if(LX_FC.sped_state==1)/*根据遥控器选择定点模式*/
		{
		rt_tar.st_data.yaw_dps = CMDvalue[0];  //航向转动角速度，度每秒，逆时针为正
		rt_tar.st_data.vel_x = CMDvalue[1];    //头向速度，厘米每秒
		rt_tar.st_data.vel_y = CMDvalue[2];    //左向速度，厘米每秒
		rt_tar.st_data.vel_z = CMDvalue[3];	 //天向速度，厘米每秒
		}
		else
		{
...
void ANO_LX_Task()
{
	...
	FC_DateUpdate();//等待发送数据更新
	BD_DataSend();//串口拓展板数据发送
//	User_GpioCheck();
	MV_DataSend();//openmv数据发送
//	FcDataToT265();//T265数据发送
	//电调输出
