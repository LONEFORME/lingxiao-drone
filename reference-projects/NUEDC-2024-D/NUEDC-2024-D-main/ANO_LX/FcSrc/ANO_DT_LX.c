...
/**************************************************************************
函数功能：T265数据通过灵活数据帧发送到上位机
入口参数：无
返回  值：无
作    者：LHB
**************************************************************************/
void t265_data_send(void)
{
	/*具体通信协议在匿名通信协议V7.PDF*/
	u8 cnt=0;
	t265_send[cnt++]=0xaa;
	...
	t265_send[cnt++]=0xf1;
	t265_send[cnt++]=0x00;
	t265_send[cnt++]=BYTE0(user_program_time);//1
	t265_send[cnt++]=BYTE1(user_program_time);

	t265_send[cnt++]=BYTE0(openmv.ifget);//2
	t265_send[cnt++]=BYTE1(openmv.ifget);
	t265_send[cnt++]=BYTE0(OutDeg);//3
	t265_send[cnt++]=BYTE1(OutDeg);
	
	t265_send[cnt++]=BYTE0(OutZcm);//4
	t265_send[cnt++]=BYTE1(OutZcm);
	t265_send[cnt++]=BYTE0(playvoice);//5
	t265_send[cnt++]=BYTE1(playvoice);
	t265_send[cnt++]=BYTE0(fly_point[38].pos_y_exp);//6
	t265_send[cnt++]=BYTE1(fly_point[38].pos_y_exp);
	
	t265_send[cnt++]=BYTE0(openmv.point0_x);//7
	t265_send[cnt++]=BYTE1(openmv.point0_x);
	t265_send[cnt++]=BYTE0(openmv.point0_y);//8
	t265_send[cnt++]=BYTE1(openmv.point0_y);
	t265_send[cnt++]=BYTE0(openmv.err_x);//9
	t265_send[cnt++]=BYTE1(openmv.err_x);


	t265_send[cnt++]=BYTE0(openmv.err_y);//10
	t265_send[cnt++]=BYTE1(openmv.err_y);
	
	t265_send[3]=cnt-4;//有效数据长度
	
	u8 check_sum1 = 0, check_sum2 = 0;
	for (u8 i = 0; i < cnt; i++)
	{
		check_sum1 += t265_send[i];//和校验
		check_sum2 += check_sum1;//附加校验
	}
	t265_send[cnt++] = check_sum1;
	t265_send[cnt++] = check_sum2;
	
	ANO_DT_LX_Send_Data(t265_send, cnt);//发送一次数据
