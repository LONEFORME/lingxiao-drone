...
static inline void General_Velocity_Data_Handle()
{
	...
	if (of_update_cnt != ano_of.of_update_cnt)
	{
		...
		{
			if(LX_FC.sped_state==0)//选择外部速度传感器来源 LHB
			{
			...
			{
			ext_sens.gen_vel.st_data.hca_velocity_cmps[0] = (s16)t265.v_x_f;
			ext_sens.gen_vel.st_data.hca_velocity_cmps[1] = (s16)t265.v_y_f;
			}
		}
		else //无效
		{
