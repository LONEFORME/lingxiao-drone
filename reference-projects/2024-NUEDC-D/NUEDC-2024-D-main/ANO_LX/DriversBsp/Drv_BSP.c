...
//	ROM_GPIOPinWrite(LED2_PORT, LED2_PIN, LED2_PIN);//r
//	ROM_GPIOPinWrite(LED1_PORT, LED1_PIN, 0);//g
//	ROM_GPIOPinWrite(LED3_PORT, LED3_PIN, 0);//b
//	MyDelayMs(400);
//	ROM_GPIOPinWrite(LED2_PORT, LED2_PIN, 0);//r
//	ROM_GPIOPinWrite(LED1_PORT, LED1_PIN, LED1_PIN);//g
//	ROM_GPIOPinWrite(LED3_PORT, LED3_PIN, 0);//b
//	MyDelayMs(400);
//	ROM_GPIOPinWrite(LED2_PORT, LED2_PIN, 0);//r
//	ROM_GPIOPinWrite(LED1_PORT, LED1_PIN, 0);//g
//	ROM_GPIOPinWrite(LED3_PORT, LED3_PIN, LED3_PIN);//b
//	MyDelayMs(400);
//	ROM_GPIOPinWrite(LED2_PORT, LED2_PIN, LED2_PIN);//r
//	ROM_GPIOPinWrite(LED1_PORT, LED1_PIN, LED1_PIN);//g
//	ROM_GPIOPinWrite(LED3_PORT, LED3_PIN, 0);//b
...
//	ROM_GPIOPinWrite(LED3_PORT, LED3_PIN, LED3_PIN);//b
	...
	MyDelayMs(800);
	//GPS接口初始化
//	Init_GPS();GPS接口用于T265 LHB
	//初始化定时中断
	DrvTimerFcInit();
	//初始化完成，返回1
	
//	OLED_Init();3个用户IO暂时保留不初始化  LHB
	User_GpioInit();
	return (1);
}
...
#define RC_NO_CHECK 1  //0：监测遥控信号；1：不检测遥控信号  1 BY LHB
//
void DrvRcInputTask(float dT_s)
{
	...
	if(rc_in.no_signal != 0 || failsafe != 0)
	{
		...
		rc_in.rc_ch.st_data.ch_[ch_5_aux1]=2000;//模拟无遥控器 BY LHB
		rc_in.rc_ch.st_data.ch_[ch_6_aux2]=1000;
		rc_in.rc_ch.st_data.ch_[ch_7_aux3]=1000;//6月底炸鸡多次，故屏蔽遥控器
		rc_in.rc_ch.st_data.ch_[ch_8_aux4]=1000;//
	}
	//不标记失控
	rc_in.fail_safe = 0;
#endif
