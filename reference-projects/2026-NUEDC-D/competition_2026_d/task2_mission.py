...
class Task2MissionDirector:
    ...
    def _sync_vision_command(
        ...
        """Generate a filtered, rate-limited visual centering command at C.
        ...
        a longer loss smoothly brakes and holds the current T265 position.
        """
        cfg = self.config
        now = float(data.now)
        valid = (
        ...
        if target_recent:
            ...
            if error_norm > cfg.c_sync_vision_deadband_m:
                ...
                desired_x = cfg.c_sync_vision_kp * error_x * active_scale
                desired_y = cfg.c_sync_vision_kp * error_y * active_scale
                desired_x, desired_y = _limit_norm(
                    desired_x,
                    desired_y,
                    cfg.c_sync_vision_max_speed_m_s,
                )
        ...
        max_delta = cfg.c_sync_vision_max_accel_m_s2 * dt
