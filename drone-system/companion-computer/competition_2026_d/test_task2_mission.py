...
def test_platform_retakeoff_rebuilds_local_frame_and_returns_in_three_points():
    cfg = Task2Config(
        platform_retakeoff_enabled=True,
        retakeoff_h_offset_x_m=-1.80,
        retakeoff_h_offset_y_m=-1.70,
    )
    director = Task2MissionDirector(cfg)
    anchor = (1.95, 2.22)
    director.begin_platform_retakeoff(now=0.0, anchor_xy_m=anchor)

    local_h_in_world = (anchor[0] - 1.80, anchor[1] - 1.70)
    ...
    expected = (
        (anchor[0] - 0.60, anchor[1] - 1.70 / 3.0),
        (anchor[0] - 1.20, anchor[1] - 2.0 * 1.70 / 3.0),
        local_h_in_world,
    )
    assert len(director._return_h_waypoints) == 3
    for actual_point, expected_point in zip(
        director._return_h_waypoints, expected
    ):
        assert actual_point == pytest.approx(expected_point)
